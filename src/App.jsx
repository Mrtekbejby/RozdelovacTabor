...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-6 md:p-12 font-sans print:bg-white print:p-0 relative">
      ...
      {/* MODÁLNÍ OKNO - PRÁZDNÉ ODDÍLY VAROVÁNÍ */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 print:hidden">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-lg w-full relative shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-slate-600 text-lg font-medium mb-8 leading-relaxed">
              Některé oddíly neobsahují <strong className="text-orange-600">žádné děti</strong>. Algoritmus sice proběhne, ale pro dokonale spravedlivé týmy je vhodné mít zastoupené všechny kategorie. Opravdu chcete pokračovat?
            </p>
            <div className="flex gap-4 justify-end">
              <button 
                onClick={() => setShowEmptyWarningModal(false)}
                className="px-6 py-4 rounded-2xl font-bold text-slate-500 hover:bg-slate-100 transition-all uppercase"
              >
                Doplnit děti
              </button>
              <button 
                onClick={potvrditRozdeleni}
                className="bg-orange-500 text-white px-8 py-4 rounded-2xl font-black hover:bg-orange-600 active:scale-95 transition-all shadow-xl uppercase flex items-center gap-2"
              >
                <Wand2 size={20} /> Přesto rozdělit
              </button>
            </div>
          </div>
        </div>
      ...
      {/* MODÁLNÍ OKNO - ROZDĚLIT DĚTI (VŠE OK) */}
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 print:hidden">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-lg w-full relative shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-slate-600 text-lg font-medium mb-8 leading-relaxed">
              Máte zkontrolovaná jména? Tento krok vygeneruje nové družiny a aktuální rozdělení přemaže. Akci nelze vrátit zpět.
            </p>
            <div className="flex gap-4 justify-end">
              <button 
              ...
              <button 
                onClick={potvrditRozdeleni}
                className="bg-yellow-400 text-yellow-950 px-8 py-4 rounded-2xl font-black hover:scale-105 active:scale-95 transition-all shadow-xl uppercase flex items-center gap-2"
              >
                <Wand2 size={20} /> Pokračovat
              </button>
            </div>
          </div>
        </div>
      ...
      {showClearModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 print:hidden">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-lg w-full relative shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-slate-600 text-lg font-medium mb-8 leading-relaxed">
              Opravdu chcete vymazat všechna zadaná jména z oddílů a zrušit aktuální družiny? Aplikace bude zcela vyčištěna a <strong className="text-red-500 font-black">tuto akci nelze vrátit zpět</strong>.
            </p>
            <div className="flex gap-4 justify-end">
              <button 
              ...
              <button 
                onClick={potvrditVymazani}
                className="bg-red-500 text-white px-8 py-4 rounded-2xl font-black hover:bg-red-600 active:scale-95 transition-all shadow-xl uppercase flex items-center gap-2"
              >
                <Trash2 size={20} /> Vymazat tabulky
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-[1600px] mx-auto bg-white shadow-2xl rounded-[3rem] overflow-hidden print:shadow-none print:border-none print:rounded-none">
        ...
        <div className="bg-slate-900 p-10 text-white flex flex-col lg:flex-row justify-between items-center gap-6 print:hidden border-b-4 border-blue-600">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-4xl font-black uppercase italic tracking-tighter leading-none">Rozdělovač družin</h1>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="bg-slate-800 p-2 pl-5 rounded-2xl flex items-center gap-4 border border-slate-700">
              ...
              <div className="flex gap-2">
                {[6, 7, 8].map(n => (
                  <button 
                    key={n}
                    onClick={() => setPocetDruzin(n)}
                    className={`px-5 py-2 rounded-xl font-black transition-all ${pocetDruzin === n ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}
                  >
                    {n}
                  </button>
                ))}
            ...
            <button 
              onClick={handleRozdelitClick} 
              className="bg-yellow-400 text-black px-12 py-6 rounded-[2rem] font-black hover:scale-105 active:scale-95 transition-all shadow-xl uppercase text-2xl flex items-center gap-4 border-b-4 border-yellow-600"
            >
              <Wand2 size={28} /> Rozdělit děti
            </button>
          </div>
        </div>
        ...
        <div className="p-8 md:p-12">
          ...
          <div className="bg-slate-50 p-4 rounded-3xl border border-slate-200 mb-10 print:hidden flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm">
            <div className="flex items-center gap-4">
              <div className="flex items-center bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                ...
                <input 
                  type="number" 
                  value={pocetTestovacichDeti} 
                  onChange={(e) => setPocetTestovacichDeti(Number(e.target.value))}
                  className="w-20 p-2 text-center font-black text-slate-700 outline-none"
                />
              </div>
              <button 
                onClick={vygenerovatData}
                className="bg-blue-100 text-blue-700 border border-blue-200 px-6 py-3 rounded-xl font-black text-sm hover:bg-blue-200 transition-all uppercase flex items-center gap-2"
              >
                <Dices size={18} /> Vygenerovat náhodně
              </button>
            </div>
            <button 
              onClick={() => setShowClearModal(true)}
              className="text-red-500 font-bold px-6 py-3 rounded-xl hover:bg-red-50 transition-all uppercase flex items-center gap-2 text-sm"
            >
              <Eraser size={18} /> Vymazat tabulky
            </button>
          </div>
          ...
          <div className="mb-16 print:hidden">
            <h2 className="text-2xl font-black text-slate-800 mb-8 uppercase flex items-center gap-3 italic tracking-tight underline decoration-blue-500 decoration-4 underline-offset-8">
              ...
              {oddily.map(o => (
                <div key={o.id} className={`rounded-[2.5rem] border-2 p-6 shadow-lg transition-transform hover:scale-[1.01] ${o.id % 2 === 0 ? 'border-pink-200 bg-pink-50/50' : 'border-blue-200 bg-blue-50/50'}`}>
                  ...
                  <div className="space-y-4">
                    <input 
                      className="w-full text-lg p-4 rounded-2xl border-2 border-white bg-white shadow-inner outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-400 font-bold transition-all" 
                      placeholder="Přidat jméno..." 
                      onKeyDown={e => {if(e.key==='Enter'){pridejDite(o.id, e.target.value); e.target.value=''}}} 
                    />
                    ...
                    <div className="space-y-2 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                      {o.deti.map((d, i) => (
                        <div key={i} className="bg-white p-3 rounded-xl flex justify-between items-center group shadow-sm border border-slate-100">
                          <span className="font-bold text-slate-700 text-lg tracking-tight truncate pr-2">
                            <span className="opacity-30 mr-2 italic text-base">{i+1}.</span>{d}
                          </span>
                          <button 
                            onClick={()=>smazDite(o.id, i)} 
                            className="text-red-300 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-all p-1 flex-shrink-0"
                          >
                            <Trash2 size={22}/>
                          </button>
                        </div>
            ...
            <div className="mt-10 pt-10 border-t-4 border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-8 print:grid-cols-2">
                {vysledneDruziny.map((druzina, idx) => {
                  ...
                  const oddilyStats = Array.from({length: 8}, (_, i) => i + 1).map(id => ({
                    id, count: druzina.filter(d => d.oddil === id).length
                  })).filter(o => o.count > 0);
                  ...
                  return (
                    <div key={idx} className={`border-2 rounded-[3rem] overflow-hidden shadow-2xl transition-all hover:scale-[1.02] flex flex-col ${barvyDruzin[idx].border} ${barvyDruzin[idx].light}`}>
                      <div className={`${barvyDruzin[idx].bg} ${barvyDruzin[idx].text} p-6 text-center font-black italic uppercase text-3xl print:bg-slate-100 print:text-black`}>
                        Družina {barvyDruzin[idx].jmeno}
                      </div>
                      <div className="p-8 space-y-3 flex-1">
                        {druzina.sort((a,b) => a.oddil - b.oddil).map((d, i) => (
                          <div key={i} className="flex justify-between items-center border-b-2 border-black/5 pb-2 last:border-0">
                            ...
                            <span className="bg-white/60 backdrop-blur-sm text-slate-500 font-black text-sm px-3 py-1.5 rounded-xl italic shadow-sm border border-black/5 flex-shrink-0">
                              #{d.rank}
                            </span>
                          </div>
                        ))}
                      ...
                      <div className="bg-black/5 p-4 border-t flex flex-col gap-3">
                        <div className="text-center text-xs font-black text-slate-500 uppercase tracking-[0.4em]">
                          CELKEM {druzina.length} DĚTÍ
                        </div>
                        ...
                        <div className="flex flex-wrap justify-center gap-1.5 mt-0.5">
                          {oddilyStats.map(oz => (
                            <span key={oz.id} className="text-[9px] font-black text-slate-500 bg-black/5 px-2 py-0.5 rounded-md uppercase">
                              O{oz.id}: {oz.count}x
                            </span>
                          ))}
                        </div>
